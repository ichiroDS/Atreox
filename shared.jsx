...
function BgColorSystem({ page }) {
  useEffect(() => {
    ...
    const b1 = document.getElementById('bg-blob-1');
    const b2 = document.getElementById('bg-blob-2');
    const b3 = document.getElementById('bg-blob-3');
    if (!b1 || !b2 || !b3) return;

    const apply = (name) => {
      const p = PALETTES[name] || PALETTES['blue-violet'];
      b1.style.backgroundColor = p.b1;
      b2.style.backgroundColor = p.b2;
      b3.style.backgroundColor = p.b3;
      b1.style.opacity = String(p.op);
      b2.style.opacity = String(p.op);
      b3.style.opacity = String(p.op * 0.7);
    };

    // Set default palette immediately
    apply('blue-violet');

    const obs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting && e.target.dataset.bgPalette) {
          apply(e.target.dataset.bgPalette);
        }
      });
    }, { threshold: 0.25 });

    const watch = () => {
      document.querySelectorAll('[data-bg-palette]').forEach(el => obs.observe(el));
    };
